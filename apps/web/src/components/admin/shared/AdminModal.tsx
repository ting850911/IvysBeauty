...
export function AdminModal({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      {/* Modal Container */}
      <div
        className={`relative w-full ${maxWidth} bg-surface border border-border/50 rounded-[32px] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300 max-h-[75vh]`}
      >
        {/* Header */}
        <div className="px-8 py-3 border-b border-border/50 flex items-center justify-between shrink-0 bg-surface/50 backdrop-blur-md">
          <h6 className="tracking-tight text-foreground">{title}</h6>
          <button
            onClick={onClose}
            ...
            {footer ? (
              ...
              <>
                ...
                <Button
                  ...
                  className={`rounded-full px-12 py-6 text-white shadow-md min-w-[140px] text-base font-medium transition-all ${confirmVariant === "default" ? "bg-primary hover:bg-primary/90" : ""
                    }`}
                >
